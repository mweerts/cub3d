...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/03 14:08:13 by mweerts           #+#    #+#             */
/*   Updated: 2020/05/03 14:24:22 by mweerts          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void	init_side_distance(t_ray *ray, t_storage *storage)
{
	if (ray->raydir_x < 0)
	{
		ray->step_x = -1;
...
static void	digital_differential_analyze(t_ray *ray, t_storage *storage, int x)
{
	while (ray->hit == 0)
	{
...
void		cast_rays(t_storage *storage, t_ray *ray)
{
	...
	while (x < storage->info->rx)
	{
		...
		init_side_distance(ray, storage);
		digital_differential_analyze(ray, storage, x);
